...
$(document).ready(function () {
    tablaData = $('#tbdata').DataTable({
        responsive: true,
        "ajax": {
            "url": '/SunatTipoDetraccion/ListaAll',
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "idTipoDetraccion", "visible": false, "searchable": false },
            { "data": "codigo" },
            { "data": "descripcion" },
            { "data": "porcentaje" },
            { "data": "condicion" },
        ...
        buttons: [
            {
                ...
                extend: 'excelHtml5',
                title: '',
                filename: 'Reporte Tipos de Detracciones',
                exportOptions: {
...
function mostarModal(modelo = MODELO_BASE) {
    $("#txtId").val(modelo.idTipoDetraccion)
...
$("#btnGuardar").click(function () {
    ...
    modelo["idTipoDetraccion"] = parseInt($("#txtId").val());
    ...
    if (modelo.idTipoDetraccion == 0) {
        fetch("/SunatTipoDetraccion/Crear", {
            method: "POST",
            body: formData
        })
            .then(response => {
            ...
            .then(responseJson => {
                if (responseJson.estado) {
                    ...
                    swal("Listo", "Tipo de Detracción fue creado", "success");
                } else {
        ...
        fetch("/SunatTipoDetraccion/Editar", {
            method: "PUT",
            body: formData
        })
            .then(response => {
            ...
            .then(responseJson => {
                if (responseJson.estado) {
                    ...
                    $("#modalData").modal("hide");
                    swal("Listo", "Tipo de Detracción fue modificado", "success");
                } else {
                    swal("Lo sentimos", responseJson.mensaje, "error");
                }
            })
...
$('#tbdata tbody').on("click", ".btn-eliminar", function () {
    ...
    swal(
        {
            title: "¿Está seguro?",
            text: `Eliminar Tipo de Detracción"${data.descripcion}"`,
            type: "warning",
        ...
        function (respuesta) {
            if (respuesta) {
                ...
                fetch(`/SunatTipoDetraccion/Eliminar?idTipoDetraccion=${data.idTipoDetraccion}`, {
                    method: "DELETE"
                })
                    .then(response => {
                    ...
                    .then(responseJson => {
                        if (responseJson.estado) {
                            ...
                            swal("Listo", "Tipo de Detracción fue eliminado", "success");
                        } else {
                            swal("Lo sentimos", responseJson.mensaje, "error");
                        }
                    })
